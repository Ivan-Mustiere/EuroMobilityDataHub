...
def test_row_count(con):
    assert con.execute("SELECT COUNT(*) FROM regularite").fetchone()[0] == 4 + 4 + 3


def test_type_ligne_values(con):
    rows = con.execute("SELECT DISTINCT type_ligne FROM regularite ORDER BY 1").fetchall()
    assert [r[0] for r in rows] == ["grande_vitesse", "intercite", "regional"]


def test_axe_type_per_source(con):
    axe_types = dict(con.execute("SELECT DISTINCT type_ligne, axe_type FROM regularite ORDER BY 1").fetchall())
    assert axe_types == {
...
def test_ter_has_region_but_no_gare(con):
    row = con.execute(
        "SELECT gare_depart, gare_arrivee, region FROM regularite WHERE type_ligne = 'regional' AND axe_label = 'Bretagne' LIMIT 1"
    ).fetchone()
...
def test_taux_ponctualite_tgv(con):
    taux = con.execute(
        "SELECT taux_ponctualite FROM regularite "
...
def test_taux_ponctualite_ter(con):
    ...
    taux = con.execute(
        "SELECT taux_ponctualite FROM regularite WHERE type_ligne='regional' AND mois='2024-01'"
    ).fetchone()[0]
    assert taux == 89.74
...
def test_taux_ponctualite_intercites(con):
    ...
    taux = con.execute(
        "SELECT taux_ponctualite FROM regularite WHERE type_ligne='intercite' AND mois='2024-01'"
    ).fetchone()[0]
    assert taux == 89.58
...
def test_taux_annulation(con):
    ...
    taux = con.execute(
        "SELECT taux_annulation FROM regularite WHERE type_ligne='regional' AND mois='2024-02'"
    ).fetchone()[0]
    assert taux == 5.0
...
def test_taux_null_when_no_circulation_or_programmation(con):
    ...
    row = con.execute(
        "SELECT taux_ponctualite, taux_annulation FROM regularite "
...
def test_retard_moyen_only_available_for_tgv(con):
    tgv_retard = con.execute(
        "SELECT retard_moyen_tous_trains_arrivee_min FROM regularite "
        "WHERE type_ligne='grande_vitesse' AND mois='2024-01' AND axe_label='PARIS -> LYON'"
    ).fetchone()[0]
    assert tgv_retard == 5.5

    ter_retard = con.execute(
        "SELECT retard_moyen_tous_trains_arrivee_min FROM regularite WHERE type_ligne='regional' LIMIT 1"
    ).fetchone()[0]
    assert ter_retard is None

    intercite_retard = con.execute(
        "SELECT retard_moyen_tous_trains_arrivee_min FROM regularite WHERE type_ligne='intercite' LIMIT 1"
    ).fetchone()[0]
...
def test_dev_sample_excludes_stale_axis_and_keeps_all_line_types(con):
    ...
    axes_by_type = dict(con.execute("SELECT DISTINCT type_ligne, axe_label FROM regularite ORDER BY 1").fetchall())
...
def test_dev_sample_keeps_only_most_recent_months(con):
    ...
    months = {r[0] for r in con.execute("SELECT DISTINCT mois FROM regularite").fetchall()}
